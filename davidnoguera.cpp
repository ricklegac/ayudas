...
int main(){
	...
	while(n<=1000 || n>=1000000 || str_n.size()%2!=0 ){ //validacion
		if(n<1000){
			cout<<"el numero es menor a 1000 vuelve a ingresar"<<endl;
			cin>>n;
			str_n= to_string(n);
		}else if(n>1000000){
			cout<<"el numero es mayor a 1000000 vuelve a ingresar"<<endl;
			cin>>n;
			str_n= to_string(n);
		}else{
	...
	cout<<"\t"<<n<<"\t"<<n2;
	//cout<<str_n2.size()<<endl;
	//cout<<str_n.size()*2<<endl;
